...
def objective_score(summary: Dict[str, float], score_mode: str) -> float:
    pf = float(summary["profit_factor_return_units"])
    mean_ret = float(summary["mean_signed_fwd_return_on_trades"])
    precision = float(summary["directional_precision_on_trades"])
    accepted = float(summary["accepted_rate"])
    bal_acc = float(summary["balanced_accuracy"])

    if score_mode == "profit_factor":
        return pf
    if score_mode == "mean_return":
        return mean_ret
    if score_mode == "precision":
        return precision
    if score_mode == "balanced_accuracy":
        return bal_acc
    if score_mode == "hybrid":
        pf_term = min(pf, 5.0) / 5.0
...
def generate_weight_candidates(step: float, allow_zero: bool) -> List[Dict[str, float]]:
    ...
    for a in range(start, units + 1):
        for b in range(start, units + 1 - a):
            c = units - a - b
            if c < start:
                continue
            weights = {"mlp": a * step, "lgbm": b * step, "hgb": c * step}
            if weights["mlp"] + weights["lgbm"] + weights["hgb"] <= 0:
                continue
            candidates.append(normalize_weights(weights["mlp"], weights["lgbm"], weights["hgb"]))

    unique = []
    seen = set()
    for w in candidates:
        key = (round(w["mlp"], 10), round(w["lgbm"], 10), round(w["hgb"], 10))
        if key not in seen:
            seen.add(key)
            unique.append(w)
    return unique
...
def walk_forward_optimize_weights(
    ...
    for fold, (train_idx, valid_idx) in enumerate(tscv.split(X), start=1):
        ...
        for weights in weight_candidates:
            ...
            score = objective_score(summary, score_mode)
        ...
        fold_results.append(best_item)

        s = best_item["summary"]
        w = best_item["weights"]
        print(
            f"Fold {fold}: BEST weights mlp={w['mlp']:.2f} lgbm={w['lgbm']:.2f} hgb={w['hgb']:.2f} "
            f"score={best_item['score']:.6f} pf={s['profit_factor_return_units']:.3f} "
            f"prec={s['directional_precision_on_trades']:.3f} accepted={s['accepted_rate']:.3f} "
            f"mean_ret={s['mean_signed_fwd_return_on_trades']:.6f}"
        )

    avg_weights = {
        "mlp": float(np.mean([x["weights"]["mlp"] for x in fold_results])),
        "lgbm": float(np.mean([x["weights"]["lgbm"] for x in fold_results])),
        "hgb": float(np.mean([x["weights"]["hgb"] for x in fold_results])),
    }
    avg_weights = normalize_weights(avg_weights["mlp"], avg_weights["lgbm"], avg_weights["hgb"])

    return fold_results, avg_weights
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", default="output_weight_search")
    ...
    p.add_argument("--allow-zero-weights", action="store_true")
    p.add_argument("--score-mode", default="hybrid",
                   choices=["hybrid", "profit_factor", "mean_return", "precision", "balanced_accuracy"])
    return p.parse_args()
...
def main() -> None:
    ...
    entry_prob_threshold, min_prob_gap = derive_decision_thresholds(
        models, train_lab, best_avg_weights, args.prob_quantile, args.margin_quantile
    )

    train_pred = classify_with_thresholds(models, train_lab, best_avg_weights, entry_prob_threshold, min_prob_gap)
    test_pred = classify_with_thresholds(models, test_lab, best_avg_weights, entry_prob_threshold, min_prob_gap)
    ...
    for item in fold_results:
        row = {
            "fold": item["fold"],
            "mlp_weight": item["weights"]["mlp"],
            "lgbm_weight": item["weights"]["lgbm"],
            "hgb_weight": item["weights"]["hgb"],
            "score": item["score"],
            "entry_prob_threshold": item["entry_prob_threshold"],
            "min_prob_gap": item["min_prob_gap"],
        }
        row.update(item["summary"])
        fold_table.append(row)
    pd.DataFrame(fold_table).to_csv(output_dir / "weight_search_fold_results.csv", index=False)
    ...
    meta = {
        ...
        "allow_zero_weights": args.allow_zero_weights,
        "score_mode": args.score_mode,
        "best_avg_weights": best_avg_weights,
        ...
        "fold_best_results": fold_results,
    }
