...
FEATURE_COLS = [
    "ret_1","ret_3","ret_5","ret_10","vol_10","vol_20",
    "dist_sma_10","dist_sma_20","zscore_20","atr_14",
]
...
def make_mlp(random_state: int = 42) -> Pipeline:
    mlp = MLPClassifier(hidden_layer_sizes=(64, 32), activation="relu", solver="adam",
                        ...
                        max_iter=300, early_stopping=False, random_state=random_state)
    return Pipeline([("scaler", StandardScaler()), ("mlp", mlp)])

def make_lgbm(random_state: int = 42) -> LGBMClassifier:
    return LGBMClassifier(objective="multiclass", num_class=3, n_estimators=350, learning_rate=0.04,
                          max_depth=6, num_leaves=31, min_child_samples=20, subsample=0.9,
                          colsample_bytree=0.9, random_state=random_state, n_jobs=-1, verbosity=-1)

def make_hgb(random_state: int = 42) -> HistGradientBoostingClassifier:
    return HistGradientBoostingClassifier(loss="log_loss", learning_rate=0.03, max_iter=400,
                                          max_leaf_nodes=31, max_depth=6, min_samples_leaf=20,
...
def fit_models(train_df: pd.DataFrame, random_state: int = 42):
    ...
    mlp = make_mlp(random_state); mlp.fit(X, y)
    lgbm = make_lgbm(random_state); lgbm.fit(X, y)
    hgb = make_hgb(random_state); hgb.fit(X, y)
    return mlp, lgbm, hgb
...
def averaged_probabilities(models, X: np.ndarray) -> np.ndarray:
    ...
    for model in models:
        ...
        aligned = np.column_stack([p[:, cmap[SELL_CLASS]], p[:, cmap[FLAT_CLASS]], p[:, cmap[BUY_CLASS]]])
...
def derive_decision_thresholds(models, train_df: pd.DataFrame, prob_quantile: float, margin_quantile: float):
    ...
    p_sell, p_flat, p_buy = proba[:, 0], proba[:, 1], proba[:, 2]
    ...
    diag = pd.DataFrame({
        "p_sell": p_sell, "p_flat": p_flat, "p_buy": p_buy,
        "best_direction_prob": best_direction_prob, "best_vs_next": best_vs_next,
...
def classify_with_thresholds(models, df: pd.DataFrame, entry_prob_threshold: float, min_prob_gap: float) -> pd.DataFrame:
    ...
    out = df.copy()
    out["p_sell"], out["p_flat"], out["p_buy"] = p_sell, p_flat, p_buy
    out["best_direction_prob"], out["prob_gap"], out["pred_class"] = best_direction_prob, prob_gap, pred
    out["trade_taken"] = out["pred_class"] != FLAT_CLASS
    out["direction_correct"] = (((out["pred_class"] == BUY_CLASS) & (out["target_class"] == BUY_CLASS))
...
def summarize_predictions(pred_df: pd.DataFrame) -> Dict[str, float]:
    ...
    if trade_mask.any():
        ...
        mean_trade_return = float(np.where(pred_df.loc[trade_mask, "pred_class"].to_numpy() == BUY_CLASS,
                                           pred_df.loc[trade_mask, "fwd_ret_h"].to_numpy(),
                                           -pred_df.loc[trade_mask, "fwd_ret_h"].to_numpy()).mean())
        accepted_trades = int(trade_mask.sum())
        accepted_rate = float(trade_mask.mean())
    else:
        directional_precision = mean_trade_return = accepted_rate = 0.0
        accepted_trades = 0
    return {
...
def walk_forward_report(train_df: pd.DataFrame, n_splits: int, label_quantile: float, prob_quantile: float, margin_quantile: float):
    ...
    for fold, (train_idx, valid_idx) in enumerate(tscv.split(X), start=1):
        ...
        accepted_rates.append(summary["accepted_rate"]); directional_precisions.append(summary["directional_precision_on_trades"])
        mean_trade_returns.append(summary["mean_signed_fwd_return_on_trades"]); ternary_accs.append(summary["ternary_accuracy"])
        balanced_accs.append(summary["balanced_accuracy"]); entry_probs.append(entry_prob_threshold); prob_gaps.append(min_prob_gap)
        print(f"Fold {fold}: barrier={barrier:.6f} entry_prob={entry_prob_threshold:.4f} gap={min_prob_gap:.4f} "
...
def save_metadata(output_dir: Path, args: argparse.Namespace, barrier: float, walk_forward: Dict[str, float],
    ...
    meta = {
        "symbol": args.symbol, "timeframe": args.timeframe, "bars": args.bars, "horizon_bars": args.horizon_bars,
        "train_ratio": args.train_ratio, "label_quantile": args.label_quantile,
        "prob_quantile": args.prob_quantile, "margin_quantile": args.margin_quantile,
        "barrier_abs_fwd_ret_h": barrier, "entry_prob_threshold": entry_prob_threshold, "min_prob_gap": min_prob_gap,
        "features": FEATURE_COLS, "class_order": CLASS_ORDER, "class_encoding": {"sell": 0, "flat": 1, "buy": 2},
        ...
        "walk_forward_train": walk_forward, "train_summary": train_summary, "test_summary": test_summary,
...
def main() -> None:
    ...
    print()
    walk_forward = walk_forward_report(train_df, args.walk_forward_splits, args.label_quantile, args.prob_quantile, args.margin_quantile)
    ...
    entry_prob_threshold, min_prob_gap, train_diag = derive_decision_thresholds(models, train_lab, args.prob_quantile, args.margin_quantile)
    ...
    save_metadata(output_dir, args, barrier, walk_forward, train_summary, test_summary, entry_prob_threshold, min_prob_gap,
                  str(train_df["time"].iloc[0]), str(train_df["time"].iloc[-1]), str(test_df["time"].iloc[0]), str(test_df["time"].iloc[-1]))
    write_run_in_mt5(output_dir, args, str(train_df["time"].iloc[0]), str(train_df["time"].iloc[-1]),
                     str(test_df["time"].iloc[0]), str(test_df["time"].iloc[-1]), entry_prob_threshold, min_prob_gap)
